...
function ecReportText() {
  ...
  return `=== BioMonitor Report ===\nDate: ${new Date().toLocaleString('zh-TW')}\nDuration: ${dur.toFixed(1)}s | Points: ${n}\nBaseBPM: ${S.base.bpm ?? '--'} | BaseRPM: ${S.base.rpm ?? '--'} | BaseGSR: ${S.base.gsr ?? '--'}\nAvg BPM: ${avgBpm.toFixed(1)} | Avg RPM: ${avgRpm.toFixed(1)}\nAvg Stress: ${avgSc.toFixed(2)} | Peak: ${maxSc.toFixed(2)}\n\nFormula (G4 piecewise-linear):\n  S_HR  = constrain((BPM - Base_BPM x1.20) / (Base_BPM x1.50 - Base_BPM x1.20) x100, 0, 100)\n  S_RPM = constrain((RPM - Base_RPM x1.25) / (Base_RPM x1.60 - Base_RPM x1.25) x100, 0, 100)\n  S_GSR = constrain((Base_GSR x0.80 - GSR) / (Base_GSR x0.80 - Base_GSR x0.50) x100, 0, 100)\n  Score = S_HR x0.40 + S_RPM x0.40 + S_GSR x0.20\n`;
}
